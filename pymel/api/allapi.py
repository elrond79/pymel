...
def toApiObject(nodeName, plugs=True, comps=True, preferPlugs=True):
    ...
    plugs : bool
        ...
        a pair of (MDagPath, MPlug) if it's a dag node, or just an MPlug if it's
        not a dag node
    comps : bool
    ...
    preferPlugs : bool
        ...
        If True, then the plug is returned, otherwise the component is.
    ...
    if len(splitName) > 1:
        ...
        if plugs:
            ...
            if plug.isNull():
                plugResult = None
            elif not comps or preferPlugs:
                # If we prefer plugs or ONLY wanted plugs, we don't need to try
                # getting a comp, and can return now!
                return plugResult
        if comps:
            # Components
            dag = MDagPath()
